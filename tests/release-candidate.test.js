// Legacy filename retained deliberately so in-place upgrades from 1.0.0-rc.1
// overwrite the old RC assertions instead of leaving a stale failing test behind.
...
test("legacy release-candidate package check follows the stable release", () => {
  assert.equal(moduleJson.version, "1.0.3.1");
  assert.equal(packageJson.version, moduleJson.version);
  assert.match(mainSource, /version: module\.version \?\? "1\.0\.3\.1"/);
  assert.match(mainSource, /apiVersion: 1/);
  assert.match(mainSource, /schemaVersion: 2/);
  assert.match(mainSource, /stability: "stable"/);
});

test("legacy release-candidate documentation check follows protected stable persistence", () => {
  ...
  assert.match(readme, /^# Campaign Forge v1\.0\.3/m);
  assert.match(apiContract, /Stable Contract/);
});

test("raw provider and storage internals remain GM-gated after stable promotion", () => {
...
test("Campaign Forge still exposes the ready discovery hook after stable promotion", () => {
...
test("all literal Campaign Forge localization keys remain present in both stable locales", () => {
